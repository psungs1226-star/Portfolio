...
두 모델 모두 C를 가장 높게 평가했지만, GPT-5.6 평가에서는 정보 위계·가독성이 B 18점, C 17점으로 나타났습니다. 따라서 모든 항목에서 우세하다고 과장하지 않고, 상세 결과와 평가 원본을 함께 공개했습니다.

단일 사례로 일반적인 우위를 주장하기보다, 이미지로 방향을 비교·선택하고 이를 코드와 검증 루프에 연결하는 방식의 가능성을 확인한 테스트입니다.

→ [A/B/C 원본 HTML과 실험 조건](Design_Fieldbook_Skill/experiments/abc-test/) · [GPT·Gemini 검수 원본](Design_Fieldbook_Skill/#다른-모델을-이용한-결과-검수)
