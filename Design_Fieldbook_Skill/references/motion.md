...
이 파일은 전부 읽는 것을 전제하지 않는다(52KB — 통째로 읽으면 그 자체로 한 라운드 분량이다). 아래에서 작업에 해당하는 절만 읽는다 — 절 안에서 다른 절을 참조하면 그때 따라간다.

**해당 절만 여는 법**(줄 번호는 파일이 바뀌면 어긋나므로 매번 grep으로 얻는다):

```
grep -n '^#\{2,3\} ' references/motion.md      # 절·소절 시작 줄 전부
```

그중 필요한 절의 시작 줄 N과 다음 헤딩 줄 M을 보고 `Read(offset=N, limit=M-N)`으로 그 구간만 읽는다.
