...
| **조합 프리셋**(스타일 아님 — 별도 층) | 3 ⭐ | `references/styles/contemporary.md` |

원본 29종 중 **7종은 금지**됐다(아래 "금지 스타일"). 상세 파일의 번호는 Fieldbook 원본 순서라 중간이 비어 있다. 마지막 행의 조합 프리셋 3종은 29종에 포함되지 않는 **다른 층위**다(아래 "조합 프리셋").

## 메인 레퍼런스 8종 — 여기서 먼저 고른다

사용자가 방향을 명시하지 않았다면 **이 8종 또는 아래 조합 프리셋 3종 안에서 먼저 후보를 찾는다.** 깊이·질감·색을 적극적으로 쓰는 계열로, 이 프로젝트가 지향하는 방향이다. 8종에는 카테고리 파일에 **구현 스펙**(그림자 공식·보더/그림자 비율·투명도 단계 등 실측값)이 붙어 있다.
...
## 조합 프리셋 3종 — 스타일과 다른 층위

**스타일이 조형 언어(무엇으로 만드는가 — 그림자·보더·radius의 문법)라면, 프리셋은 레이아웃 + 타이포 + 색 + 모션을 한 판으로 묶은 검증된 조합(어떻게 한 페이지로 짜는가)이다.** 2020년대 제품·마케팅 화면의 관용구라 29종 안에 스타일로 편입하면 어색하고, 계보 5종에도 넣지 않는다. 위 8종과 **동등한 후보**이며, 방향 미지정 시 후보 선택지는 "메인 레퍼런스 8종 **또는 프리셋 3종**"이다. 프리셋은 스타일 하나를 고른 것과 같은 지위이므로 `/* plan */`의 `스타일:` 칸에 `preset:refined-product`처럼 적는다.

| 프리셋 | 태그 | 한 줄 | DNA | 파일 |
|---|---|---|---|---|
| 정제된 프로덕트 | `refined-product` | 다크·중립 베이스 위 알파 층으로 만든 밀도 높은 제품 화면 | 3-1-5-2-2-1-2-1 | contemporary.md |
| 에디토리얼 키네틱 | `editorial-kinetic` | 뷰포트를 점유하는 대형 활자 + 모션 전부를 활자 하나에 | 5-4-5-0-0-0-2-2 | contemporary.md |
| 벤토 쇼케이스 | `bento-showcase` | 셀 크기·배경·밀도가 위계를 만드는 기능 소개 한 판 | 3-3-4-1-5-2-3-3 | contemporary.md |

**각 프리셋에는 "결정적 동작" 2~3개가 붙어 있고, 그것이 금지 스타일과 갈리는 유일한 지점이다** — 상세 파일에서 반드시 확인한다. 프리셋과 스타일을 섞을 수도 있다(예: `bento-showcase`의 판 위에 `material-you`의 tonal container). 이때도 충돌 축은 한쪽을 이긴 것으로 명시한다.
...
**조합 프리셋은 이 금지의 우회로가 아니다** — `refined-product`는 `minimal`과, `bento-showcase`는 `flat`·`corporate`와 표면적으로 닮았다. 프리셋 상세의 **결정적 동작을 하나라도 빠뜨리면 그 산출물은 프리셋이 아니라 위 금지 스타일과 동일하게 판정한다.** 이름을 선언하는 것은 이행이 아니다.
...
- **contemporary.md**(조합 프리셋 — 스타일 아님, 계보에 등장하지 않는다): `refined-product` `editorial-kinetic` `bento-showcase`
...
- 금지: 프리셋 이름을 `/* plan */`에 적고 결정적 동작(알파 층·모노 수치·100ms 상태 전이 / 뷰포트급 헤드라인·활자 전용 모션 / 셀 크기 3종·배경 3종·밀도 차등)을 빠뜨림 — 금지 스타일 채택과 동일하게 판정한다
- 필수: 메인 레퍼런스 8종 또는 조합 프리셋 3종 중 하나(또는 명시적 하이브리드)를 고르고, DNA 8축 중 최소 2–3개 축에서 뚜렷한 값(0 또는 4–5)을 만든다. 구현 스펙의 **공식**(그림자 겹수·방향, 보더/오프셋 비율, 투명도 단계)은 정확히 따르되 **색은 프로젝트 팔레트로 재현**
