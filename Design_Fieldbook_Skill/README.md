...
> 디자인 자료를 요약한 프롬프트 모음이 아니라, **방향 판단 → 디자인 토큰 계산 → 구현 → 독립 미적 검토 → 코드·렌더링 검증**을 하나의 작업 흐름으로 강제하는 실행 시스템입니다.
...
| `verify.py` | 계획 선언, 접근성, 타입 스케일, 간격, 스타일 커밋, 모션 남용 등 정적 검사 |
...
├── references/              # 디자인 원칙·컬러·타입·레이아웃·접근성·모션
│   └── styles/              # 29개 스타일의 계보·관찰점·구현 관계
├── scripts/
...
스킬 핵심부는 `SKILL.md`와 참고자료·스크립트·데모·프로브를 포함한 **25개 파일, 약 6,090줄**로 구성되어 있습니다.
